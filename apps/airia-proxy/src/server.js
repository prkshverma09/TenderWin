...
const baseUrl = process.env.AIRIA_API_URL ?? process.env.VITE_AIRIA_API_URL ?? 'https://api.airia.ai';
const apiKey = process.env.AIRIA_API_KEY ?? process.env.VITE_AIRIA_API_KEY ?? '';
const agentId = process.env.AIRIA_AGENT_ID ?? process.env.VITE_AIRIA_AGENT_ID ?? '';
const app = express();
...
app.post('/airia', async (req, res) => {
    ...
    const pathTemplate = process.env.AIRIA_INVOKE_PATH ?? process.env.VITE_AIRIA_INVOKE_PATH ?? '/v1/PipelineExecution/Multipart/{agentId}';
    ...
    try {
        ...
        let confidence;
        let sources;
        const extractTextFromArray = (arr) => {
            const parts = [];
            for (const s of arr) {
                ...
                if (typeof v === 'string' && v.trim())
                    ...
                    for (const key of ['Value', 'value', 'Content', 'content', 'Text', 'text']) {
                        if (typeof o[key] === 'string' && o[key].length > 20) {
                            parts.push(o[key].trim());
                            break;
                        }
                    }
                }
        ...
        if (Array.isArray(raw)) {
            ...
            if (typeof obj.result === 'string')
                text = obj.result;
            else if (obj.text != null && typeof obj.text === 'string')
                text = obj.text;
            else if (obj.content != null && typeof obj.content === 'string')
                text = obj.content;
            else if (Array.isArray(obj.result))
                text = extractTextFromArray(obj.result);
            else if (Array.isArray(obj.value))
                text = extractTextFromArray(obj.value);
            else if (Array.isArray(obj.steps))
                text = extractTextFromArray(obj.steps);
                ...
                if (typeof res.text === 'string')
                    ...
                    text = extractTextFromArray(res.value);
            }
            if (obj.confidence != null && typeof obj.confidence === 'number')
                confidence = obj.confidence;
            if (obj.report != null && typeof obj.report === 'object' && obj.report.confidence != null) {
